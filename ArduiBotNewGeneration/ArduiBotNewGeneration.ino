...
unsigned long checkingTimeUS = 200; 	// temps entre 2 relev�s US // min 40ms
...
void actionServosBalai( ){
	...
	if( posBalai + tempPas < posMinBalai || posBalai + tempPas > posMaxBalai ){
		...
		// si on est qd meme en dehors, peut-etre que l'on vient de changer de mode et que le servo est de l'autre cot�...
...
void prepareServoBalai( int posMin, int posMax, int pas, unsigned long freq ){
	...
	// on positionne le balai en d�but de cone
	if( abs(posBalai - posMinBalai) > abs( posBalai - posMaxBalai ) ){
		delta = abs( posBalai - posMinBalai ); // nb de degr� pour arriver en position
	...
	if( mode != balaiRunningMode ){
		switch( mode ){
			case BALAI_MODE_ARRET :
				...
				prepareServoBalai( 0, 180, 30, 10000 ); // a l'arret toute les 10sec on scan le p�rim�tre complet
				if( isDebug )
					Serial.println( "BALAI_MODE_ARRET" );
				break;
			case BALAI_MODE_AVANCE :
				balaiRunningMode = BALAI_MODE_AVANCE;
				prepareServoBalai( 60, 120, 30, 500 ); // en avancant, tt les 500ms on scan un petit p�rim�tre
				if( isDebug )
 ...
 */
void actionVoltage(){
	// on r�cup�re la valeur du capteur
	int sensorValue;  
	...
	// si nous avons pass� le nombre d'�chantillonage
	if( voltageSamplingNb > voltageSamples ){
		// on calcule la moyenne
		sensorValue = voltageValuesSum / voltageSamples; 
		voltage = GAIN * sensorValue * ADC_REF / 1023.00; // tir� de l'exemple officiel
		// reset
		voltageSamplingNb = 0;
		voltageValuesSum = 0;
		// affichage
